...
export function setupFileSystemHandlers() {
  ...
  ipcMain.handle("fs:writeFileBuffer", async (event, filePath, bufferArray) => {
    try {
...
export function setupEdgeHandlers() {
  ipcMain.handle("edge:callBoomyBuilder", async (_, buildRequest: any) => {
    try {
      ...
      const debugPath = path.join(process.cwd(), "..", "BoomyBuilder", "bin", "Debug", "net8.0", platformsMap[platformKey], exeName);
      const releasePath = path.join(process.cwd(), "..", "BoomyBuilder", "bin", "Release", "net8.0", platformsMap[platformKey], exeName);
      ...
      request["milo_template_path"] = path.join(path.dirname(builderExePath), "Assets", "template.milo_xbox");
      request["barks_template_path"] = path.join(path.dirname(builderExePath), "Assets", "barks_template.milo_xbox");
      request["songs_dta_path"] = path.join(path.dirname(builderExePath), "Assets", "songs.dta");
      ...
      return await new Promise((resolve) => {
        ...
        child.on("close", async (code) => {
          ...
          try {
            await fs.writeFile("build.log", `STDIN:\n${JSON.stringify(request)}\n\nSTDOUT:\n${stdout}\n\nSTDERR:\n${stderr}`);
          } catch (logErr) {
...
export function setupAudioHandlers() {
  ...
  ipcMain.handle("audio:convertToMogg", async (event, audioPath: string) => {
    try {
      ...
      const possiblePaths = [path.join(process.cwd(), "..", "BoomyDeps", "makemogg", "makemogg.exe"), path.join(__dirname, "makemogg.exe"), path.join(resourcesPath, "makemogg", "makemogg.exe")];
      ...
      console.log(`Converting ${audioPath} to ${outputPath} using ${makemoggPath}`);
      ...
      const result = await new Promise<{
        ...
        const process = spawn(makemoggPath, [audioPath, "-m", outputPath], {
          stdio: ["pipe", "pipe", "pipe"],
        });
