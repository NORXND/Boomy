...
export function OpenSongDialog({ open, setOpen, songPath }: { open: boolean; setOpen: (open: boolean) => void; songPath: string }) {
  ...
  const handleSongCreation = async () => {
    ...
    const moveVersionExists = await window.electronAPI.pathExists(pathJoin(filePath, ".boomy"));

    if (moveVersionExists) {
      const dotBoomyContent = await window.electronAPI.readFile(pathJoin(filePath, ".boomy"));
    ...
    if (boomyExists) {
      toast.error("Project already exists!", {
        description: "There is a Boomy project in this directory. Try opening it instead.",
      });
    }
      ...
      <DialogContent className="sm:max-w-md">
        ...
        <div className="flex items-center gap-2">
          <div className="flex flex-1 gap-2 flex-row">
            ...
            <Input id="file" placeholder="milo-move-library" onChange={(e) => setFilePath(e.target.value)} value={filePath} />
