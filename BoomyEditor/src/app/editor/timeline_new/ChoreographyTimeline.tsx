...
export const ChoreographyTimeline = React.memo(
  function ChoreographyTimeline({ timelineData, currentTime, isPlaying, autoScroll, calculateCursorPosition, handleSeek, timeToBeat, timelineScrollRef, pixelsPerBeat, trackHeaderWidth, addUndoAction }: ChoreographyTimelineProps) {
    ...
    const getMoveEventsForMeasure = useCallback(
      (difficulty: "supereasy" | "easy" | "medium" | "expert", measure: number) => {
        if (!currentSong) return [];

        // Map our difficulty to the song store's difficulty format
        const difficultyMap: Record<string, "supereasy" | "easy" | "medium" | "expert"> = {
          supereasy: "supereasy",
          easy: "easy",
          medium: "medium",
          expert: "expert",
        ...
        if (difficulty === "supereasy") {
          ...
          events = currentSong.timeline[mappedDifficulty as "easy" | "medium" | "expert"].moves;
        }

        // Filter events that match this measure
        return events.map((event, index) => ({ ...event, originalIndex: index })).filter((event) => event.measure === measure);
      },
      [currentSong],
    );
    ...
    const handleDrop = useCallback(
      (e: React.DragEvent, difficulty: "supereasy" | "easy" | "medium" | "expert", measure: Measure) => {
        e.preventDefault();
        setDragOverCell(null);
        ...
        try {
          ...
          if (dragData.type === "move-clip") {
            ...
            if (currentSong) {
              ...
              const eventIndex = events.findIndex((event) => event.measure === measure.number);
              if (eventIndex !== -1) {
    ...
    const handleCellClick = useCallback(
      (difficulty: "supereasy" | "easy" | "medium" | "expert", measure: number, e: React.MouseEvent) => {
        e.stopPropagation();
        ...
        // If ctrl key is pressed, toggle this cell in selection
          ...
          const index = selectedCells.findIndex((c) => c.difficulty === difficulty && c.measure === measure);
    ...
    const handleCopy = useCallback(() => {
      ...
      const copied = [];
      const sortedCells = [...selectedCells].sort((a, b) => a.measure - b.measure);
      ...
      for (const cell of sortedCells) {
        ...
        const moveEvent = events.find((event) => event.measure === cell.measure);
        if (moveEvent) {
          copied.push({
    ...
    const handlePaste = useCallback(
      async (targetDifficulty: "supereasy" | "easy" | "medium" | "expert", targetMeasure: number) => {
        ...
        if (!clipboardPayload || clipboardPayload.boomy !== "copypaste1" || clipboardPayload.type !== "move" || !Array.isArray(clipboardPayload.data)) {
          return;
        }
        ...
        for (const item of clipboardPayload.data) {
          ...
          let existingEvent;
          if (mappedDifficulty === "supereasy") {
            existingEventIndex = currentSong.supereasy.findIndex((event) => event.measure === newMeasure);
            existingEvent = currentSong.supereasy[existingEventIndex];
          } else {
            existingEventIndex = currentSong.timeline[mappedDifficulty].moves.findIndex((event) => event.measure === newMeasure);
            existingEvent = currentSong.timeline[mappedDifficulty].moves[existingEventIndex];
          }
      ...
      [clipboardData, currentSong, addMoveEvent, removeMoveEvent, addUndoAction],
    );
    ...
    const handleDelete = useCallback(() => {
      ...
      const toDelete = selectedCells
        .map((cell) => {
          ...
          const eventIndex = events.findIndex((event) => event.measure === cell.measure);
          if (eventIndex !== -1) {
            return {
    ...
    const handleMoveEventClick = useCallback(
      (difficulty: "supereasy" | "easy" | "medium" | "expert", eventIndex: number, e: React.MouseEvent) => {
        ...
        if (e.target instanceof HTMLElement && e.target.closest("[data-delete-button]")) {
          removeMoveEvent(mappedDifficulty, eventIndex);
          return;
        }
        ...
        if (mappedDifficulty === "supereasy") {
          ...
          event = currentSong.timeline[mappedDifficulty].moves[eventIndex];
        }
    ...
    const useMoveImageAndDataCache = (moveLibPath: string) => {
      ...
      const getMoveImageAndData = useCallback(
        async (moveKey: string) => {
          if (imageCache.current[moveKey] && dataCache.current[moveKey]) {
            return {
    ...
    const handleGenerateLowerDifficulties = async (track: "supereasy" | "easy" | "medium") => {
      ...
      if (result) {
        ...
        for (const move of result) {
          ...
          let existingEvent;
          if (track === "supereasy") {
            existingEventIndex = currentSong.supereasy.findIndex((event) => event.measure === move.measure);
            existingEvent = currentSong.supereasy[existingEventIndex];
          } else {
            existingEventIndex = currentSong.timeline[track].moves.findIndex((event) => event.measure === move.measure);
            existingEvent = currentSong.timeline[track].moves[existingEventIndex];
          }
          if (existingEventIndex !== -1 && existingEvent) {
    ...
    useEffect(() => {
      const preload = async () => {
        ...
        for (const track of ["supereasy", "easy", "medium", "expert"] as const) {
          for (const measure of timelineData.measures) {
            const events = getMoveEventsForMeasure(track, measure.number);
            for (const event of events) {
              const moveKey = `${event.move_origin}/${event.move_song}/${event.move}`;
              if (!(moveKey in images)) {
                const result = await getMoveImageAndData(moveKey);
    ...
    }, [timelineData, getMoveEventsForMeasure, moveLibPath, getMoveImageAndData]);
      ...
      <div className="flex h-full">
        <div className="flex-1 overflow-hidden flex flex-col">
          <div ref={timelineScrollRef} className="overflow-auto relative flex-1">
            ...
            {(["supereasy", "easy", "medium", "expert"] as const).map((track) => (
              <div key={track} className="h-[90px] border-b flex">
                {/* Track Label */}
                <div className="flex-shrink-0 bg-muted/30 flex items-center justify-between px-3 border-r" style={{ width: trackHeaderWidth }}>
                  <span className="text-sm font-medium capitalize">{track}</span>

                  {track !== "expert" && <WandSparkles onClick={() => handleGenerateLowerDifficulties(track)} width={16} height={16}></WandSparkles>}
                </div>
                ...
                {timelineData.measures.map((measure) => {
                  ...
                  const isSelected = selectedCells.some((cell) => cell.difficulty === track && cell.measure === measure.number);
                  ...
                  return (
                    <ContextMenu key={cellKey}>
                      <ContextMenuTrigger>
                        <div
                          className={`flex flex-shrink-0 border-r transition-colors cursor-pointer relative items-center justify-center group min-h-full ${
                            isSelected ? "bg-primary/30 border-primary" : isHighlighted ? "bg-primary/20 border-primary" : "hover:bg-muted/20"
                          }`}
                          style={{
                            width: `${measure.beatCount * pixelsPerBeat}px`,
                          }}
                          onClick={(e) => handleCellClick(track, measure.number, e)}
                          onDragOver={(e) => handleCellDragOver(e, cellKey)}
                          onDragLeave={handleDragLeave}
                          onDrop={(e) => handleDrop(e, track, measure)}
                        >
                          {/* Events */}
                          {events.length > 0 ? (
                                ...
                                return (
                                  <div
                                    ...
                                    className={`flex-1 border rounded p-1 flex flex-col items-center justify-center transition-colors min-h-0 ${
                                      isSelected ? "bg-primary/40 border-primary" : "bg-primary/20 border-primary/40 hover:bg-primary/30"
                                    }`}
                                    onClick={(e) => handleMoveEventClick(track, moveEvent.originalIndex, e)}
                                  >
                                    <div className="w-16 h-8 bg-muted rounded mb-1 flex items-center justify-center flex-shrink-0 overflow-hidden">
                                      {(() => {
                                        const moveKey = `${moveEvent.move_origin}/${moveEvent.move_song}/${moveEvent.move}`;
                                        const imgSrc = moveImages[moveKey];
                                        if (imgSrc) {
                                          return <img src={imgSrc} alt={moveEvent.move} className="object-contain w-full h-full" draggable={false} />;
                                        }
                                    ...
                                    <button
                                      ...
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        removeMoveEvent(track, moveEvent.originalIndex);
                                      }}
                                    >
                            ...
                            <div className="w-full h-full flex items-center justify-center">
                              <div className={`w-3 h-3 rounded-full ${isSelected ? "bg-primary/50" : "bg-muted/30 group-hover:bg-primary/50"} transition-colors`} />
                            </div>
                          )}
                        </div>
                      </ContextMenuTrigger>
                      ...
                      <ContextMenuContent>
                        <ContextMenuItem onClick={() => handleCopy()} disabled={selectedCells.length === 0}>
                          <Copy className="mr-2 h-4 w-4" />
                          Copy
                        </ContextMenuItem>
                        <ContextMenuItem onClick={() => handlePaste(track, measure.number)} disabled={clipboardData.length === 0}>
                          <Clipboard className="mr-2 h-4 w-4" />
                          Paste
                        </ContextMenuItem>
                        <ContextMenuItem onClick={() => handleDelete()} disabled={selectedCells.length === 0} className="text-destructive">
