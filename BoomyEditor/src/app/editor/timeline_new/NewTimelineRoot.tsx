...
import { Play, Pause, Square, SkipBack, SkipForward, Lock, Unlock, Layers, Undo2, Redo2 } from "lucide-react";
...
import { CameraEvent, DancerFaceEvent, MoveEvent, TempoChange } from "@/types/song";
...
export function NewTimelineRoot({ mode = "choreography", timelineAdditionalProps }: NewTimelineRootProps) {
  // Create a unique ID for this component instance
  const instanceId = useRef(`timeline-${Math.random().toString(36).slice(2)}`).current;
  ...
  const { audioPath, currentSong, saveSong, isLoading: isSaving, totalMeasures, setTotalMeasures } = useSongStore();

  // Use shared timeline context
  const { currentTime, setCurrentTime: setSharedCurrentTime, isPlaying, setIsPlaying: setSharedIsPlaying, duration, setDuration: setSharedDuration, setTimelineData: setSharedTimelineData } = useTimelineContext();
  ...
  const loadAudioFile = useCallback(async () => {
    ...
    try {
      ...
      const audioBuffer = await window.electronAPI.readFileBuffer(audioPath);
  ...
  const calculateCursorPosition = useCallback(
    (time?: number): number => {
      if (!timelineData || !timelineData.beatMap) return TRACK_HEADER_WIDTH;
      ...
      for (let i = 0; i < beatMap.length; i++) {
        if (beatMap[i] > targetTime) {
          ...
          const progress = (targetTime - prevBeatTime) / segmentDuration;
          beat = i - 1 + progress;
          break;
        }
  ...
  const updatePlaybackPosition = useCallback(() => {
    ...
    if (autoScroll && timelineScrollRef.current && timelineData) {
      ...
      const totalTimelineWidth = TRACK_HEADER_WIDTH + timelineData.measures.reduce((total, measure) => total + measure.beatCount * pixelsPerBeat, 0);

      // Constrain to valid scroll range
      const maxScrollLeft = Math.max(0, totalTimelineWidth - containerWidth);
      const newScrollLeft = Math.max(0, Math.min(idealScrollLeft, maxScrollLeft));
  ...
  const startPlaybackLoop = useCallback(() => {
    ...
    if (!audioElementRef.current || (mountedRef.current.size > 1 && Array.from(mountedRef.current).pop() !== instanceId)) {
      return;
    }
    ...
    const updateLoop = () => {
      ...
      if (!localAudioRef.current || localAudioRef.current.paused || (mountedRef.current.size > 1 && Array.from(mountedRef.current).pop() !== currentInstanceId)) {
        return;
      }
  ...
  const handlePlay = useCallback(async () => {
    ...
    if (mountedRef.current.size > 1 && Array.from(mountedRef.current).pop() !== instanceId) {
      return;
    }
      ...
      if (audio.readyState < 2) {
        await new Promise((resolve, reject) => {
          const onCanPlay = () => {
            ...
            if (mountedRef.current.size <= 1 || Array.from(mountedRef.current).pop() === currentInstanceId) {
              resolve(void 0);
            } else {
              reject(new Error("Component instance no longer active"));
            }
          };
  ...
  const getCurrentMeasure = useCallback(() => {
    ...
    return timelineData.measures.find((measure) => currentTime >= measure.startTime && currentTime < measure.startTime + measure.duration);
  ...
  const timeToBeat = useCallback(
    (time: number): number => {
      ...
      if (time > lastTime && timelineData.tempoChanges.length > 0) {
        const lastTempo = timelineData.tempoChanges[timelineData.tempoChanges.length - 1];
        const duration = time - lastTime;
  ...
  const calculateTimelineData = useCallback((): TimelineData | null => {
    ...
    const tempoList: TempoChange[] = sortedTempoChanges.length > 0 && sortedTempoChanges[0].measure === 0 ? sortedTempoChanges : [{ measure: 0, bpm: 120 }, ...sortedTempoChanges];

    const timeSignatures: TimeSignature[] = [{ numerator: 4, denominator: 4, ticks: 0 }];
    ...
    for (let beat = 0; beat < totalBeats; beat++) {
      ...
      while (tempoIndex + 1 < tempoList.length && currentTime >= tempoList[tempoIndex + 1].measure) {
        tempoIndex++;
      }
    ...
    for (let i = 0; i < maxMeasures; i++) {
      ...
      if (beatMap[startBeat] === undefined || beatMap[endBeat] === undefined) {
        console.warn(`Could not generate measure ${i + 1} due to missing beat map data.`);
        continue;
      }
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      if (audioElementRefForCleanup.current) {
        ...
        if (audioElementRefForCleanup.current.src && audioElementRefForCleanup.current.src.startsWith("blob:")) {
    ...
    switch (last.type) {
      case "move:add": {
        ...
        if (last.data.track === "supereasy") {
          idx = useSongStore.getState().currentSong?.supereasy.findIndex((ev: any) => ev.measure === last.data.event.measure);
        } else {
          idx = useSongStore.getState().currentSong?.timeline?.[last.data.track]?.moves.findIndex((ev: any) => ev.measure === last.data.event.measure);
        }

        if (idx) {
          useSongStore.getState().removeMoveEvent(last.data.track, idx);
        }

        break;
      }
      case "move:remove": {
        useSongStore.getState().addMoveEvent(last.data.track, last.data.event);
        break;
      }
      case "move:bulkadd": {
        let moves;
        if (last.data.track === "supereasy") {
          moves = useSongStore.getState().currentSong?.supereasy || [];
        } else {
          moves = useSongStore.getState().currentSong?.timeline?.[last.data.track].moves || [];
        }

        last.data.events.forEach((ev: any) => {
          const idx = moves.findIndex((move: any) => move.measure === ev.measure);
          if (idx !== -1) {
            useSongStore.getState().removeMoveEvent(last.data.track, idx);
          }
        });
      ...
      case "camera:add":
        const idx = useSongStore.getState().currentSong?.timeline?.[last.data.track]?.cameras.findIndex((ev: any) => ev.beat === last.data.event.beat);

        useSongStore.getState().removeCameraEvent(last.data.track, idx);
        break;
      case "camera:remove":
        useSongStore.getState().addCameraEvent(last.data.track, last.data.event);
        break;
      case "camera:bulkadd":
        last.data.events.forEach((ev) => {
          const idx = useSongStore.getState().currentSong?.timeline?.[last.data.track]?.cameras.findIndex((cam: any) => cam.beat === ev.beat);

          useSongStore.getState().removeCameraEvent(last.data.track, idx);
        });
        break;
      case "camera:bulkremove":
        last.data.events.forEach((ev) => useSongStore.getState().addCameraEvent(last.data.track, ev));
        break;
      case "drums:trackadd": {
        const currDrums = useSongStore.getState().currentSong?.drums;
        if (!currDrums) return;

        const newDrumsEvents = currDrums.filter((_, i) => i !== last.data.event.index);
      ...
      case "drums:drumadd": {
        ...
        const newDrumsEvents = currDrums.map((drum) => {
          if (drum.sound === last.data.track) {
            return {
              ...drum,
              events: drum.events.filter((ev) => ev !== last.data.event.beatIndex),
            };
          }
          return drum;
        });
      ...
      case "events:add": {
        switch (last.data.track) {
          case "song":
            const songIdx = useSongStore.getState().currentSong?.events.findIndex((ev) => ev.beat === last.data.event.beat && ev.type === last.data.event.type);
            useSongStore.getState().removeEvent(songIdx);
            break;
          case "battle":
            const battleIdx = useSongStore.getState().currentSong?.battleSteps.findIndex((step) => step.measure === last.data.event.measure);
            useSongStore.getState().removeBattleStep(battleIdx);
            break;
          case "partybattle":
            const partyBattleIdx = useSongStore.getState().currentSong?.partyBattleSteps.findIndex((step) => step.measure === last.data.event.measure);
            useSongStore.getState().removepartyBattleSteps(partyBattleIdx);
            break;
          case "party":
            const partyIdx = useSongStore.getState().currentSong?.partyJumps.findIndex((ev) => ev.measure === last.data.event.measure);
      ...
      case "visemes:add": {
        ...
        const visemes = useSongStore.getState().currentSong?.dancerFaces?.[track as "easy" | "medium" | "expert"] || [];
        const idx = visemes.findIndex((v: DancerFaceEvent) => v.beat === event.beat && v.viseme === event.viseme && v.value === event.value);
        if (idx !== -1) {
          useSongStore.getState().removeDancerFaceEvent(track as "easy" | "medium" | "expert", idx);
        }
        break;
      }
      case "visemes:remove": {
      ...
      case "visemes:bulkadd": {
        ...
        const visemes = useSongStore.getState().currentSong?.dancerFaces?.[track] || [];
  ...
  const redo = useCallback(() => {
    ...
    switch (last.type) {
      ...
      case "camera:bulkremove": {
        ...
        const cameras = useSongStore.getState().currentSong?.timeline?.[last.data.track]?.cameras || [];
        last.data.events.forEach((ev: any) => {
          const idx = cameras.findIndex((cam: any) => cam.beat === ev.beat);
          if (idx !== -1) {
            useSongStore.getState().removeCameraEvent(last.data.track, idx);
          }
        });
      ...
      case "events:add": {
        switch (last.data.track) {
          ...
          case "battle":
            ...
            break;
          case "partybattle":
            useSongStore.getState().addpartyBattleSteps(last.data.event);
            break;
          case "party":
  ...
  const calculateCursorPositionMemo = useCallback(calculateCursorPosition, [calculateCursorPosition]);
  ...
  const timelineProps = useMemo(
    ...
    [timelineData, currentTime, isPlaying, autoScroll, calculateCursorPositionMemo, handleSeekMemo, timeToBeatMemo, timelineScrollRef, instanceId, addUndoAction],
  );
      ...
      {loading ? (
        <div className="h-full flex items-center justify-center">
          <div className="text-center">
            <div className="text-sm text-muted-foreground">Loading MIDI data...</div>
          </div>
        </div>
      ) : error || !timelineData ? (
        <div className="h-full flex items-center justify-center">
          <div className="text-center text-destructive">
            <div className="text-sm font-medium">Error loading MIDI</div>
        ...
        <>
          ...
          <div className="flex-shrink-0 p-4 border-b bg-background">
            <div className="flex items-center justify-between mb-4">
                ...
                <div className="text-sm text-muted-foreground flex items-center gap-2">
                  {mode === "events" && (
                    <div className="flex items-center gap-2">
                      <button className="px-2 py-1 rounded bg-muted border hover:bg-muted/70" onClick={() => setTotalMeasures(Math.max(1, timelineData.measures.length - 1))} title="Decrease measures">
                        -
                      </button>
                      <span>{timelineData.measures.length} measures</span>
                      <button className="px-2 py-1 rounded bg-muted border hover:bg-muted/70" onClick={() => setTotalMeasures(timelineData.measures.length + 1)} title="Increase measures">
                        +
                      </button>
                    </div>
                  )}
                  <span className="ml-2">{timelineData.totalDuration.toFixed(2)}s duration</span>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2">
                {/* Undo/Redo Buttons */}
                <button onClick={undo} className="p-2 rounded hover:bg-muted transition-colors" title="Undo" disabled={undoStack.length === 0}>
                  <Undo2 className="w-4 h-4" />
                </button>
                <button onClick={redo} className="p-2 rounded hover:bg-muted transition-colors" title="Redo" disabled={redoStack.length === 0}>
                  <Redo2 className="w-4 h-4" />
                </button>

                <div className="w-px h-6 bg-border mx-2" />

                <button onClick={() => handleSeek(0)} className="p-2 rounded hover:bg-muted transition-colors" title="Skip to beginning">
                  <SkipBack className="w-4 h-4" />
                </button>

                {isPlaying ? (
                  <button onClick={handlePause} className="p-2 rounded hover:bg-muted transition-colors" title="Pause">
                    <Pause className="w-4 h-4" />
                  </button>
                ) : (
                  <button onClick={handlePlay} className="p-2 rounded hover:bg-muted transition-colors" title="Play" disabled={!audioReady}>
                    <Play className="w-4 h-4" />
                  </button>
                )}

                <button onClick={handleStop} className="p-2 rounded hover:bg-muted transition-colors" title="Stop">
                ...
                <button
                  ...
                  className={`p-2 rounded transition-colors ${autoScroll ? "bg-primary text-primary-foreground hover:bg-primary/90" : "hover:bg-muted"}`}
                ...
                {getCurrentMeasure() && <div className="ml-4 text-sm text-muted-foreground">Measure {getCurrentMeasure()?.number}</div>}
              </div>
            </div>
          ...
          {/* Timeline Content */}
          {mode === "choreography" && <ChoreographyTimeline {...timelineProps} trackHeaderWidth={TRACK_HEADER_WIDTH} pixelsPerBeat={pixelsPerBeat} {...timelineAdditionalProps} />}
          {mode === "cameras" && <CameraShotsTimeline {...timelineProps} trackHeaderWidth={TRACK_HEADER_WIDTH} pixelsPerBeat={pixelsPerBeat} {...timelineAdditionalProps} />}
          {mode === "events" && <EventsTimeline {...timelineProps} tempoChanges={timelineData.tempoChanges} trackHeaderWidth={TRACK_HEADER_WIDTH} pixelsPerBeat={pixelsPerBeat} {...timelineAdditionalProps} />}
          {mode === "drums" && <DrumsTimeline {...timelineProps} trackHeaderWidth={TRACK_HEADER_WIDTH} pixelsPerBeat={pixelsPerBeat} {...timelineAdditionalProps} />}
          {mode === "visemes" && <DancerFacesTimeline {...timelineProps} trackHeaderWidth={TRACK_HEADER_WIDTH} pixelsPerBeat={pixelsPerBeat} {...timelineAdditionalProps} />}
        </>
      )}
    </div>
  );
