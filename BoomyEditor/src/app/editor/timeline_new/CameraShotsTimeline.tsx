...
import { Copy, Clipboard, Trash, X, ScissorsLineDashed, Camera, WandSparkles } from "lucide-react";
...
export const CameraShotsTimeline = React.memo(
  function CameraShotsTimeline({ timelineData, currentTime, isPlaying, autoScroll, calculateCursorPosition, handleSeek, timeToBeat, timelineScrollRef, pixelsPerBeat, trackHeaderWidth, addUndoAction }: CameraShotsTimelineProps) {
    ...
    const removeCameraEvent = useSongStore((state) => state.removeCameraEvent);
    ...
    const getCameraEventsForBeat = useCallback(
      (difficulty: "easy" | "medium" | "expert", beat: number) => {
        ...
        return events.map((event, index) => ({ ...event, originalIndex: index })).filter((event) => event.beat === beat);
      },
      [currentSong],
    );
    ...
    const handleDrop = useCallback(
      (e: React.DragEvent, difficulty: "easy" | "medium" | "expert", beat: number) => {
        e.preventDefault();
        setDragOverCell(null);
        ...
        try {
          ...
          if (dragData.type === "camera-position") {
            ...
            if (currentSong) {
              const events = currentSong.timeline[difficulty].cameras;
              const eventIndex = events.findIndex((event) => event.beat === beat);
              if (eventIndex !== -1) {
    ...
    const handleCellClick = useCallback(
      (difficulty: "easy" | "medium" | "expert", beat: number, e: React.MouseEvent) => {
        e.stopPropagation();
        ...
        // If ctrl key is pressed, toggle this cell in selection
          ...
          const index = selectedCells.findIndex((c) => c.difficulty === difficulty && c.beat === beat);
    ...
    const handleCopy = useCallback(() => {
      ...
      const copied = [];
      const sortedCells = [...selectedCells].sort((a, b) => a.beat - b.beat);
      const firstBeat = sortedCells[0].beat;

      for (const cell of sortedCells) {
        const events = currentSong.timeline[cell.difficulty].cameras;
        const cameraEvent = events.find((event) => event.beat === cell.beat);
        if (cameraEvent) {
          copied.push({
    ...
    const handlePaste = useCallback(
      async (targetDifficulty: "easy" | "medium" | "expert", targetBeat: number) => {
        ...
        if (!clipboardPayload || clipboardPayload.boomy !== "copypaste1" || clipboardPayload.type !== "camera" || !Array.isArray(clipboardPayload.data)) {
          return;
        }
        ...
        for (const item of clipboardPayload.data) {
          ...
          const existingEventIndex = currentSong.timeline[targetDifficulty].cameras.findIndex((event) => event.beat === newBeat);
    ...
    const handleDelete = useCallback(() => {
      ...
      const toDelete = selectedCells
        .map((cell) => {
          ...
          const eventIndex = events.findIndex((event) => event.beat === cell.beat);
          if (eventIndex !== -1) {
            return {
    ...
    const handleCameraEventClick = useCallback(
      (difficulty: "easy" | "medium" | "expert", eventIndex: number, e: React.MouseEvent) => {
        ...
        const event = currentSong?.timeline[difficulty].cameras[eventIndex];
        if (event) {
    ...
    const handleGenerate = async (track: "easy" | "medium" | "expert") => {
      ...
      if (result) {
        ...
        for (const camera of result) {
          ...
          let existingEvent;
          existingEventIndex = currentSong.timeline[track].cameras.findIndex((event) => event.beat === camera.beat);
          existingEvent = currentSong.timeline[track].cameras[existingEventIndex];
          ...
          <div ref={timelineScrollRef} className="overflow-auto relative flex-1">
            ...
            <div className="h-8 border-b bg-background flex sticky top-0 z-10">
              ...
              {timelineData.measures.map((measure) => (
                ...
                >
                  {/* Measure label */}
                  <div className="h-8 px-2 flex items-center justify-center bg-muted/50 cursor-pointer hover:bg-muted/70" onClick={() => handleSeek(measure.startTime)} title={`Jump to measure ${measure.number}`}>
            ...
            {(["easy", "medium", "expert"] as const).map((track) => (
              <div key={track} className="h-[90px] border-b flex">
                ...
                <div className="flex-shrink-0 border-r bg-muted/30 flex items-center justify-between px-3" style={{ width: trackHeaderWidth }}>
                  ...
                  <WandSparkles onClick={() => handleGenerate(track)} width={16} height={16}></WandSparkles>
                </div>
                    ...
                    >
                      ...
                      <div className="flex h-full">
                        {beatsForMeasure.map((beat, beatIdx) => {
                          const events = getCameraEventsForBeat(track, beat);
                          const cellKey = `${track}-${beat}`;
                          const isHighlighted = dragOverCell === cellKey;
                          const isSelected = selectedCells.some((cell) => cell.difficulty === track && cell.beat === beat);

                          return (
                            <ContextMenu key={`${measure.number}-${beatIdx}`}>
                              <ContextMenuTrigger>
                                <div
                                  className={`flex justify-center items-center border-r transition-colors cursor-pointer relative group min-h-full ${
                                    isSelected ? "bg-blue-500/30 border-blue-500" : isHighlighted ? "bg-blue-500/20 border-blue-500" : "hover:bg-muted/20"
                                  }`}
                                  style={{
                                    width: `${pixelsPerBeat}px`,
                                  }}
                                  onClick={(e) => handleCellClick(track, beat, e)}
                                  onDragOver={(e) => handleCellDragOver(e, cellKey)}
                                  onDragLeave={handleDragLeave}
                                  onDrop={(e) => handleDrop(e, track, beat)}
                                >
                                  {/* Events */}
                                  {events.length > 0 ? (
                                        ...
                                        return (
                                          <div
                                            ...
                                            className={`flex-1 border rounded p-1 flex flex-col items-center justify-center transition-colors min-h-0 ${
                                              isSelected ? "bg-blue-500/40 border-blue-500" : "bg-blue-500/20 border-blue-500/40 hover:bg-blue-500/30"
                                            }`}
                                            onClick={(e) => handleCameraEventClick(track, cameraEvent.originalIndex, e)}
                                          >
                                            <div className="w-6 h-6 bg-blue-500/10 rounded mb-1 flex items-center justify-center flex-shrink-0">
                                              <Camera className="h-3 w-3 text-blue-500" />
                                            </div>
                                            <div className="text-xs font-medium truncate w-full leading-tight text-center">{cameraEvent.camera}</div>
                                            ...
                                            <button
                                              ...
                                              onClick={(e) => {
                                                e.stopPropagation();
                                                removeCameraEvent(track, cameraEvent.originalIndex);
                                              }}
                                            >
                                    ...
                                    <div className="w-full h-full flex items-center justify-center">
                                      <div className={`w-2 h-2 rounded-full ${isSelected ? "bg-blue-500/50" : "bg-muted/30 group-hover:bg-blue-500/50"} transition-colors`} />
                                    </div>
                                  )}
                                </div>
                              </ContextMenuTrigger>
                              ...
                              <ContextMenuContent>
                                ...
                                <ContextMenuItem onClick={() => handlePaste(track, beat)} disabled={clipboardData.length === 0}>
  ...
  (prevProps, nextProps) => {
    ...
    return prevProps.currentTime === nextProps.currentTime && prevProps.isPlaying === nextProps.isPlaying && prevProps.autoScroll === nextProps.autoScroll && prevProps.timelineData.measures.length === nextProps.timelineData.measures.length;
  },
);
