...
type BattleEventType = "battle_reset" | "player1_solo" | "player2_solo" | "minigame_start" | "minigame_idle" | "minigame_end";

// Party Jump event types for the Party Jump track
type PartyJumpEventType = "start" | "end";

// Styles for party jump events
const partyJumpEventStyles: Record<PartyJumpEventType, { bg: string; border: string; text: string; label: string }> = {
  start: {
    bg: "bg-yellow-500/20",
...
export const EventsTimeline = React.memo(
  function EventsTimeline({ timelineData, currentTime, isPlaying, autoScroll, calculateCursorPosition, handleSeek, timelineScrollRef, tempoChanges, pixelsPerBeat, trackHeaderWidth, addUndoAction }: EventsTimelineProps) {
    renderCount++;
    ...
    if (process.env.NODE_ENV === "development") {
      console.debug("[EventsTimeline] Render", renderCount, {
        ...
        timelineDataHash: timelineData ? JSON.stringify(timelineData).length : 0,
      });
    }
    ...
    const [selectedEventType, setSelectedEventType] = useState<EventType>("music_start");
    const [selectedBeat, setSelectedBeat] = useState<number | null>(null);
    const [openBpmDialog, setOpenBpmDialog] = useState(false);
    const [bpmDialogMeasure, setBpmDialogMeasure] = useState<Measure | null>(null);
    ...
    const [selectedBattleType, setSelectedBattleType] = useState<BattleEventType>("battle_reset");
    const [selectedBattleBeat, setSelectedBattleBeat] = useState<number | null>(null);

    // Party Jump event dialog state
    const [showAddPartyJumpDialog, setShowAddPartyJumpDialog] = useState(false);
    const [selectedPartyJumpType, setSelectedPartyJumpType] = useState<PartyJumpEventType>("start");
    const [selectedPartyJumpBeat, setSelectedPartyJumpBeat] = useState<number | null>(null);

    // Party Battle event dialog state
    const [showAddPartyBattleDialog, setShowAddPartyBattleDialog] = useState(false);
    const [selectedPartyBattleType, setSelectedPartyBattleType] = useState<BattleEventType>("battle_reset");
    const [selectedPartyBattleBeat, setSelectedPartyBattleBeat] = useState<number | null>(null);
    ...
    const [freestyleWarning, setFreestyleWarning] = useState<string | null>(null);

    const handleAddEvent = useCallback(() => {
      if (selectedBeat === null || !selectedEventType || !currentSong) return;

      // Check for freestyle event placement
      if (selectedEventType === "freestyle") {
        // Find music_end event beat
        const musicEndEvent = (currentSong.events || []).find((e) => e.type === "music_end");
        if (musicEndEvent) {
          // Find measure index for selectedBeat
          const selectedMeasureIdx = timelineData.measures.findIndex((m) => selectedBeat >= m.startBeat && selectedBeat < m.startBeat + m.beatCount);
          // Find measure index for music_end event
          const musicEndMeasureIdx = timelineData.measures.findIndex((m) => musicEndEvent.beat >= m.startBeat && musicEndEvent.beat < m.startBeat + m.beatCount);
          if (musicEndMeasureIdx !== -1 && selectedMeasureIdx !== -1 && musicEndMeasureIdx - selectedMeasureIdx <= 8) {
            setFreestyleWarning("Freestyle must be placed at least 9 measures before Music End.");
            return;
          }
        }
      }
      ...
      const existingIdx = events.findIndex((e) => e.beat === selectedBeat);
      if (existingIdx !== -1) {
    ...
    }, [selectedBeat, selectedEventType, currentSong, timelineData.measures]);

    // Handle click to delete event - use removeEvent function from songStore
    const handleEventClick = useCallback((eventIndex: number, e: React.MouseEvent) => {
      e.stopPropagation();
      ...
      addUndoAction({
        ...
        data: {
          track: "song",
          event: useSongStore.getState().currentSong?.events[eventIndex],
        },
      });
    }, []);
    ...
    const handleAddPartyJumpEventClick = useCallback((measureNumber: number) => {
    ...
    const handleAddPartyJumpEvent = useCallback(() => {
      if (selectedPartyJumpBeat === null || !selectedPartyJumpType) return;

      // Remove any existing party jump event at this measure
      const idx = partyJumps.findIndex((ev) => ev.measure === selectedPartyJumpBeat);
      if (idx !== -1) {
        removePartyJump(idx);
      }
    ...
    }, [selectedPartyJumpBeat, selectedPartyJumpType, addPartyJump, partyJumps, removePartyJump]);

    // Remove a party jump event
    const handleRemovePartyJumpEvent = useCallback(
      (measureNumber: number) => {
        const idx = partyJumps.findIndex((ev) => ev.measure === measureNumber);
        if (idx !== -1) {
          removePartyJump(idx);
        }
      },
    ...
    const handleAddPartyBattleEventClick = useCallback((measureNumber: number) => {
    ...
    const handleAddPartyBattleEvent = useCallback(() => {
      if (selectedPartyBattleBeat === null || !selectedPartyBattleType) return;

      // Remove any existing party battle event at this measure
      const idx = partyBattleSteps.findIndex((ev) => ev.measure === selectedPartyBattleBeat);
      if (idx !== -1) {
    ...
    const lastBeat = timelineData.measures.reduce((total, measure) => total + measure.beatCount, 0);
    ...
    const eventStyles: Record<EventType, { bg: string; border: string; text: string; label: string }> = {
      music_start: {
        bg: "bg-green-500/20",
    ...
    const battleEventStyles: Record<BattleEventType, { bg: string; border: string; text: string; label: string }> = {
      battle_reset: {
        bg: "bg-red-500/20",
        border: "border-red-500",
        text: "text-red-500",
    ...
    }, [bpmDialogMeasure, bpmDialogValue, tempoChanges, addTempoChange, updateTempoChange]);
    ...
    const totalWidth = trackHeaderWidth + timelineData.totalBeats * pixelsPerBeat;
    ...
    }, [currentTime, autoScroll, calculateCursorPosition, timelineScrollRef]);
    ...
    return (
      <div className="flex h-full flex-col">
        ...
        <div className="p-4 border-b">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            {Object.entries(eventStyles).map(([type, style]) => (
              <div key={type} className={`px-2 py-1 rounded-full ${style.bg} ${style.border} ${style.text} flex items-center gap-1`}>
                {getEventIcon(type as EventType)}
                <span>{style.label}</span>
              </div>
            ))}
          </div>
          <p className="text-xs text-muted-foreground mt-2">Click on a beat to add an event. The 'end' event is automatically placed at the last beat.</p>
        </div>
        ...
        <div className="p-4 border-b">
          ...
          <div className="flex flex-wrap gap-2 text-xs">
            {Object.entries(battleEventStyles).map(([type, style]) => (
              <div key={type} className={`px-2 py-1 rounded-full ${style.bg} ${style.border} ${style.text} flex items-center gap-1`}>
                <span>{style.label}</span>
              </div>
            ))}
          </div>
          <p className="text-xs text-muted-foreground mt-2">Battle events are special timeline events for solos and minigames. Click a beat in the Battle track to add one.</p>
        </div>

        {/* Party Jump info panel */}
        <div className="p-4 border-b">
          <h3 className="text-sm font-medium mb-2">Party Jump Events</h3>
          <div className="flex flex-wrap gap-2 text-xs">
            {Object.entries(partyJumpEventStyles).map(([type, style]) => (
              <div key={type} className={`px-2 py-1 rounded-full ${style.bg} ${style.border} ${style.text} flex items-center gap-1`}>
                <span>{style.label}</span>
              </div>
            ))}
          </div>
          <p className="text-xs text-muted-foreground mt-2">Party Jump events are special timeline events for party jump sections. Click a beat in the Party Jump track to add one.</p>
        </div>

        {/* Timeline */}
        <div ref={timelineScrollRef} className="overflow-auto relative flex-1">
          {/* Playback Cursor */}
          ...
          <div className="h-8 border-b bg-background flex sticky top-0 z-10">
            {/* Track Labels Header */}
            <div className="flex-shrink-0 border-r bg-background flex items-center px-3" style={{ width: trackHeaderWidth }}>
            ...
            {timelineData.measures.map((measure) => {
              ...
              const tempoChangeIdx = tempoChanges.findIndex((tc) => tc.measure === measure.startTime);
              ...
              return (
                <div
                  ...
                  style={{
                    width: `${measure.beatCount * pixelsPerBeat}px`,
                  }}
                >
                  {/* Measure label and BPM change */}
                  <div className="h-8 px-2 flex items-center justify-center bg-muted/50 cursor-pointer hover:bg-muted/70 gap-2" onClick={() => handleSeek(measure.startTime)}>
                    <div className="text-xs font-medium text-center">
                      <div>{measure.number}</div>
                      <div className="text-muted-foreground">{measure.bpm.toFixed(0)}</div>
                    </div>
                    <Button
                      variant="ghost"
                      size="icon"
                    ...
                    {hasTempoChange && (
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          removeTempoChange(tempoChangeIdx);
                        }}
                      >
              ...
              return (
                ...
                >
                  ...
                  <div className="flex h-full">
                    {beatsForMeasure.map((beat, beatIdx) => {
                      const events = getEventsForBeat(beat);
                      const cellKey = `events-${beat}`;
                      const isHighlighted = dragOverCell === cellKey;
                      const isLastBeat = beat === lastBeat - 1;
                      ...
                      return (
                        <div
                          ...
                          className={`flex justify-center items-center border-r transition-colors cursor-pointer relative group min-h-full ${
                            isHighlighted ? "bg-primary/20 border-primary" : isLastBeat ? "bg-purple-500/10" : "hover:bg-muted/20"
                          }`}
                          style={{
                            width: `${pixelsPerBeat}px`,
                          }}
                          onClick={() => handleSeek(beat)} // <-- Always seek on click
                          onDoubleClick={() => handleAddEventClick(beat)} // Optional: double click to add event
                        >
                          ...
                          {events.length > 0 ? (
                            <div className="w-full h-full p-1 flex flex-col gap-1 overflow-hidden">
                              {events.slice(0, 1).map((event, idx) => {
                                ...
                                return (
                                  <div key={`${event.originalIndex}-${idx}`} className={`flex-1 border rounded p-1 flex flex-col items-center justify-center transition-colors ${style.bg} ${style.border}`}>
                                    <div className={`rounded-full p-1 ${style.bg} flex items-center justify-center`}>{getEventIcon(eventType)}</div>
                                    <div className={`text-xs font-medium ${style.text} text-center mt-1`}>{style.label}</div>
                                      ...
                                      <button
                                        ...
                                        onClick={(e) => {
                                          ...
                                          if (event.originalIndex !== undefined) {
                                            handleEventClick(event.originalIndex, e);
                                          }
                                        }}
                                      >
                          ...
                          {/* Beat number tooltip */}
                          <div className="absolute -bottom-6 left-1/2 transform -translate-x-1/2 text-[9px] text-muted-foreground opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none">Beat {beat + 1}</div>
                        </div>
                      );
                    })}
                  </div>
            ...
            {timelineData.measures.map((measure) => {
              ...
              return (
                <div
                  ...
                  onClick={() => handleAddBattleEventClick(measure.number)}
                >
                  {battleEvents.length > 0 ? (
                    <div className="w-full h-full p-1 flex flex-col gap-1 overflow-hidden">
                      {battleEvents.map((event, idx) => {
                        const style = battleEventStyles[event.type as BattleEventType];
                        return (
                          <div
                          ...
                          >
                            <div className={`rounded-full p-1 ${style.bg} flex items-center justify-center`}>{/* Optionally add an icon */}</div>
                            <div className={`text-xs font-medium ${style.text} text-center mt-1`}>{style.label}</div>
                            <button
                              className="absolute top-1 right-1 opacity-0 group-hover:opacity-100 hover:bg-destructive/10 p-1 rounded-full"
                              onClick={(e) => {
                                e.stopPropagation();
                                handleRemoveBattleEvent(event.originalIndex);
                              }}
                            >
                              <X className="h-3 w-3 text-destructive" />
                            </button>
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleAddBattleEventClick(measure.number);
                        }}
                      >
                        <Plus className="h-3 w-3" />
                      </Button>
          ...
          <div className="h-[90px] border-b flex">
            {/* Track Label */}
            <div className="flex-shrink-0 border-r bg-muted/30 flex items-center px-3" style={{ width: trackHeaderWidth }}>
              <span className="text-sm font-medium">Party Jump</span>
            </div>
            {/* One cell per measure */}
            {timelineData.measures.map((measure) => {
              const event = getPartyJumpEventForMeasure(measure.number);
              const style = event ? partyJumpEventStyles[event.type as PartyJumpEventType] : null;
              return (
                <div
                  key={measure.number}
                  className="flex-shrink-0 border-r relative flex items-center justify-center group"
                  ...
                  onClick={() => handleAddPartyJumpEventClick(measure.number)}
                >
                  {event ? (
                    <div className={`flex-1 border rounded p-1 flex flex-col items-center justify-center transition-colors ${style?.bg} ${style?.border} w-full h-full`} onClick={(e) => e.stopPropagation()}>
                      <div className={`rounded-full p-1 ${style?.bg} flex items-center justify-center`}>{/* Optionally add an icon */}</div>
                      <div className={`text-xs font-medium ${style?.text} text-center mt-1`}>{style?.label}</div>
                      <div className="flex gap-1 mt-1">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleRemovePartyJumpEvent(measure.number);
                          }}
                        >
                          <X className="h-3 w-3 text-destructive" />
                        </Button>
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleAddPartyJumpEventClick(measure.number);
                        }}
                      >
                        <Plus className="h-3 w-3" />
                      </Button>
            ...
            {timelineData.measures.map((measure) => {
              ...
              return (
                <div
                  ...
                  onClick={() => handleAddPartyBattleEventClick(measure.number)}
                >
                  {partyBattleEvents.length > 0 ? (
                    <div className="w-full h-full p-1 flex flex-col gap-1 overflow-hidden">
                      {partyBattleEvents.map((event, idx) => {
                        const style = battleEventStyles[event.type as BattleEventType];
                        return (
                          <div
                            key={`${event.originalIndex}-${idx}`}
                          ...
                          >
                            ...
                            <button
                              ...
                              onClick={(e) => {
                                e.stopPropagation();
                                handleRemovePartyBattleEvent(event.originalIndex);
                              }}
                            >
                              <X className="h-3 w-3 text-destructive" />
                            </button>
                    ...
                    <div className="w-full h-full flex items-center justify-center">
                      <Button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleAddPartyBattleEventClick(measure.number);
                        }}
                      >
                        <Plus className="h-3 w-3" />
                      </Button>
          ...
          <Dialog open={showAddEventDialog} onOpenChange={setShowAddEventDialog}>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                <DialogTitle>Add Event at Beat {selectedBeat !== null ? selectedBeat + 1 : ""}</DialogTitle>
              </DialogHeader>
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  ...
                  <Select value={selectedEventType} onValueChange={(value) => setSelectedEventType(value as EventType)}>
                    ...
                    <SelectContent>
                      <SelectItem value="music_start">Music Start</SelectItem>
                      <SelectItem value="preview">Preview</SelectItem>
                      <SelectItem value="freestyle">Freestyle</SelectItem>
                      <SelectItem value="music_end">Music End</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              ...
              {freestyleWarning && <div className="mt-4 text-sm text-red-500">{freestyleWarning}</div>}
            </DialogContent>
          </Dialog>

          {/* Add Battle Event Dialog */}
          <Dialog open={showAddBattleDialog} onOpenChange={setShowAddBattleDialog}>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                <DialogTitle>Add Battle Event at Beat {selectedBattleBeat !== null ? selectedBattleBeat + 1 : ""}</DialogTitle>
                <DialogDescription>Select a battle event type to add at this beat.</DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium">Battle Event Type</label>
                  <Select value={selectedBattleType} onValueChange={(value) => setSelectedBattleType(value as BattleEventType)}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select battle event type" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="battle_reset">Battle Reset</SelectItem>
                      <SelectItem value="player1_solo">Player 1 Solo</SelectItem>
                      <SelectItem value="player2_solo">Player 2 Solo</SelectItem>
                      <SelectItem value="minigame_start">Minigame Start</SelectItem>
                      <SelectItem value="minigame_idle">Minigame Idle</SelectItem>
                      <SelectItem value="minigame_end">Minigame End</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setShowAddBattleDialog(false)}>
                  Cancel
                </Button>
                <Button onClick={handleAddBattleEvent}>Add Battle Event</Button>
              </div>
            </DialogContent>
          </Dialog>

          {/* Add Party Jump Event Dialog */}
          <Dialog open={showAddPartyJumpDialog} onOpenChange={setShowAddPartyJumpDialog}>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                <DialogTitle>Add Party Jump Event at Beat {selectedPartyJumpBeat !== null ? selectedPartyJumpBeat + 1 : ""}</DialogTitle>
                <DialogDescription>Select a party jump event type to add at this beat.</DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium">Party Jump Event Type</label>
                  <Select value={selectedPartyJumpType} onValueChange={(value) => setSelectedPartyJumpType(value as PartyJumpEventType)}>
                    <SelectTrigger>
                      <SelectValue placeholder="Select party jump event type" />
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="start">Party Jump Start</SelectItem>
                      <SelectItem value="end">Party Jump End</SelectItem>
                    </SelectContent>
                  </Select>
                </div>
              </div>
              <div className="flex justify-end gap-2">
                ...
                <Button
                  onClick={() => {
                    const existing = partyJumps.find((ev) => ev.measure === selectedPartyJumpBeat);
                    if (existing) {
                      setShowAddPartyJumpDialog(false);
                    } else {
                ...
                >
                  {partyJumps.find((ev) => ev.measure === selectedPartyJumpBeat) ? "Edit Party Jump Event" : "Add Party Jump Event"}
                </Button>
              </div>
            </DialogContent>
          </Dialog>
          ...
          <Dialog open={showAddPartyBattleDialog} onOpenChange={setShowAddPartyBattleDialog}>
            <DialogContent className="sm:max-w-[425px]">
              <DialogHeader>
                <DialogTitle>Add Party Battle Event at Measure {selectedPartyBattleBeat !== null ? selectedPartyBattleBeat + 1 : ""}</DialogTitle>
                <DialogDescription>Select a party battle event type to add at this measure.</DialogDescription>
              </DialogHeader>
              <div className="space-y-4 py-4">
                <div className="space-y-2">
                  <label className="text-sm font-medium">Party Battle Event Type</label>
                  <Select value={selectedPartyBattleType} onValueChange={(value) => setSelectedPartyBattleType(value as BattleEventType)}>
                    <SelectTrigger>
              ...
              <div className="flex justify-end gap-2">
                <Button variant="outline" onClick={() => setShowAddPartyBattleDialog(false)}>
                  Cancel
                </Button>
                <Button onClick={handleAddPartyBattleEvent}>Add Party Battle Event</Button>
              </div>
            </DialogContent>
          </Dialog>
          ...
          <Dialog open={openBpmDialog} onOpenChange={setOpenBpmDialog}>
            <DialogContent>
              <DialogHeader>
                <DialogTitle>Change BPM for Measure {bpmDialogMeasure?.number ?? ""}</DialogTitle>
              </DialogHeader>
              <div className="py-4">
