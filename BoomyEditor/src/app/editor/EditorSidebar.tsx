...
export function EditorSidebar() {
			...
			if (performTestsBeforeBuilding) {
				if (!testsCompleted) {
					...
					if (hasErrors) {
						...
						toast.success(
							'Tests passed! Proceeding with build...',
							{ id: 'test-run' }
						);
					}
				} else {
	...
	const buildAndSaveWithCustomPath = async (
		...
		try {
			...
			const buildResult = await window.electronAPI.callBoomyBuilder(
				buildRequest
			);
	...
	const handleExportAndSave = async () => {
		...
		try {
			...
			const songData = await window.electronAPI.readJsonFile(
				songJsonPath
			);
			...
			if (songData.meta?.cover_image_path) {
				...
				coverImagePath = metaCoverPath.includes(':')
					? metaCoverPath
					: join(songPath, metaCoverPath);
			} else {
			...
			if (
				coverImagePath &&
				(await window.electronAPI.pathExists(coverImagePath))
			) {
			...
			<Dialog open={buildDialogOpen} onOpenChange={setBuildDialogOpen}>
				<DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
					<DialogHeader>
						...
						<DialogDescription>
							Configure build settings for your song.
						</DialogDescription>
					</DialogHeader>
					<div className="grid gap-4 py-4">
						...
						<div className="grid grid-cols-4 items-center gap-4">
							...
							<div className="col-span-3 flex gap-2">
								<Input
									...
									placeholder="Select build output directory..."
									className="flex-1"
								/>
								<Button
									onClick={handleSelectBuildPath}
									variant="outline"
								>
									Browse
								</Button>
						...
						<div className="flex items-center space-x-2">
							<Checkbox
								id="testsBeforeBuilding"
								checked={performTestsBeforeBuilding}
								onCheckedChange={(checked) =>
									setPerformTestsBeforeBuilding(
										checked === true
									)
								}
							/>
							<Label htmlFor="testsBeforeBuilding">
								Perform tests before building (default)
							</Label>
						</div>
										...
										{Object.entries(testResults).map(
											([testName, result]) => (
												...
												>
													...
													<div className="text-sm text-muted-foreground whitespace-pre-wrap ml-6">
														{result.output}
													</div>
												</div>
											)
					...
					<DialogFooter>
						...
						<Button
							...
							disabled={!buildPath || isRunningTests}
						>
