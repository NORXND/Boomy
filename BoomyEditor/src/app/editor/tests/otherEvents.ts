...
export async function checkDrumsTracks(song: SongState, utils: TestingUtils): Promise<TestResult> {
  ...
  const emptyEvents = drums.filter((track) => !track.events || track.events.length === 0);
...
export async function checkSongMetaImageSize(song: SongState, utils: TestingUtils): Promise<TestResult> {
  const imagePath = path.join(song.songPath, song.currentSong?.meta?.cover_image_path);
  if (!imagePath) {
    return {
      status: TestResultType.warning,
...
export async function checkBamPhraseCount(song: SongState, utils: TestingUtils): Promise<TestResult> {
