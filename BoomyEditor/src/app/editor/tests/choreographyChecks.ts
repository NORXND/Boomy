...
export async function isFirstMeasureNotEmpty(
	song: SongState,
	utils: TestingUtils
): Promise<TestResult> {
	const supereasy = song.currentSong?.supereasy.filter(
		(move) => move.measure === 1
	);

	const easy = song.currentSong?.timeline.easy.moves.filter(
		(move) => move.measure === 1
	);

	const medium = song.currentSong?.timeline.medium.moves.filter(
		(move) => move.measure === 1
	);

	const expert = song.currentSong?.timeline.expert.moves.filter(
		(move) => move.measure === 1
	);
...
export async function hasFinishingMove(
	...
	for (const diff of difficulties) {
		...
		const hasFinishing = lastMoves.some(async (m) => {
			const moveData = await utils.checkMove(
				song,
				m.move,
				m.move_song,
				m.move_origin
			...
			return (
				flags.includes('final_pose') ||
				moveData?.final_pose === true ||
				moveData?.isFinishingMove === true
			);
		});
...
export async function areRestsOnlyAtBeginningOrAfterFinishing(
	song: SongState,
	utils: TestingUtils
): Promise<TestResult> {
	const difficulties = [
	...
	for (const diff of difficulties) {
		const moves = [...diff.moves].sort(
			(a, b) => a.measure - b.measure || a.measure - b.measure
		);
		...
		for (let i = 0; i < moves.length; i++) {
			...
			const flags = utils.decodeFlags(
				moveData.clips[moves[i].clip].flags || 0
			);
			...
			if (isRest) {
				...
				if (foundScored && !foundFinishing) {
					errors.push(
						`${diff.name}: Rest move at measure ${moves[i].measure}, beat ${moves[i].measure} is not at the beginning or after a finishing move.`
					);
				}
			} else {
				foundScored = true;
...
export async function noScoredMovesAfterFinishing(
	song: SongState,
	utils: TestingUtils
): Promise<TestResult> {
	const difficulties = [
	...
	for (const diff of difficulties) {
		...
		for (let i = 0; i < moves.length; i++) {
			...
			if (foundFinishing && isScored) {
				errors.push(
					`${diff.name}: Scored move at measure ${moves[i].measure}, beat ${moves[i].measure} appears after a finishing move.`
				);
			}
...
export async function isDifficultyHierarchyRespected(
	song: SongState,
	utils: TestingUtils
): Promise<TestResult> {
