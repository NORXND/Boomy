...
function getChoreoMoveForMeasure(
	song: SongState,
	difficulty: string,
	measure: number
) {
...
export async function arePracticeMovesUsedAtMostTwice(
	song: SongState,
	utils: TestingUtils
): Promise<TestResult> {
	const difficulties = [
	...
	for (const diff of difficulties) {
		...
		const measureToMoveClip: Record<
			number,
			{ move: string; clip: string }
		> = {};
		...
		const overused = Object.entries(moveClipCount).filter(
			([, count]) => count > 2
		);
		if (overused.length > 0) {
			errors.push(
				`${diff.name}: moves+clips used more than twice: ${overused
					.map(([k, v]) => `${k} (${v} times)`)
					.join(', ')}`
			);
		}
...
export async function arePracticeSectionsConsecutive(
	song: SongState,
	utils: TestingUtils
): Promise<TestResult> {
	const difficulties = [
			...
			for (let i = 1; i < measures.length; i++) {
				if (measures[i] !== measures[i - 1] + 1) {
					errors.push(
						`${diff.name} section ${
							s + 1
						} has a gap between measures ${measures[i - 1]} and ${
							measures[i]
						}`
					);
				}
...
export async function arePracticeSectionsProgressing(
	song: SongState,
	utils: TestingUtils
): Promise<TestResult> {
	const difficulties = [
		...
		for (let s = 0; s < diff.sections.length; s++) {
			...
			if (first <= lastEnd) {
				errors.push(
					`${diff.name} section ${
						s + 1
					} starts at measure ${first} but previous section ended at ${lastEnd}`
				);
			}
...
export async function isLastPracticeSectionFinishing(
	song: SongState,
	utils: TestingUtils
): Promise<TestResult> {
	const difficulties = [
	...
	for (const diff of difficulties) {
		...
		if (!move) {
			errors.push(
				`${diff.name}: No choreography move found for measure ${lastMeasure} in last practice section.`
			);
			continue;
		}
		...
		if (!isFinishing) {
			errors.push(
				`${diff.name}: Last move in the last practice section is not a finishing pose.`
			);
		}
	}
...
export async function isPracticeDifficultyHierarchyRespected(
	song: SongState,
	utils: TestingUtils
): Promise<TestResult> {
