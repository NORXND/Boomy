...
export async function checkSongEvents(song: SongState, utils: TestingUtils): Promise<TestResult> {
  const events: EventWithType[] = (song.currentSong?.events as EventWithType[]) ?? [];
  const errors: string[] = [];
  ...
  const freestyleStart = events.filter((e) => e.type === "freestyle").sort((a, b) => a.beat - b.beat)[0];
  ...
  if (!freestyleStart) {
    ...
    if (measureDiff < 9) {
      errors.push(`Freestyle section starts too late (only ${measureDiff} beats before song_end, should be at least 9).`);
    }
  }
  ...
  for (const e of [...previewEvents, ...freestyleEvents]) {
    if (e.beat <= songStartBeat) {
      errors.push(`${e.type} event at beat ${e.beat} is before song_start.`);
    }
  ...
  if (!battleStart) errors.push("Missing battle_reset event in battle steps.");

  // Minigame start/end matching (battleSteps only)
  const minigameStarts = battleEvents.filter((e) => e.type === "minigame_start").sort((a, b) => a.measure - b.measure);
  const minigameEnds = battleEvents.filter((e) => e.type === "minigame_end").sort((a, b) => a.measure - b.measure);
  ...
  if (minigameStarts.length === 0 && minigameEnds.length === 0) {
    ...
    errors.push("Mismatched minigame_start and minigame_end events in battle steps.");
  } else {
    for (let i = 0; i < minigameStarts.length; i++) {
      if (minigameStarts[i].measure >= minigameEnds[i].measure) {
        errors.push(`minigame_start at measure ${minigameStarts[i].measure} does not end properly at ${minigameEnds[i].measure}.`);
      }
      // Check for overlap: next start must not be before previous end
      if (i > 0 && minigameStarts[i].measure < minigameEnds[i - 1].measure) {
        errors.push(`minigame_start at measure ${minigameStarts[i].measure} overlaps previous minigame.`);
      }
      // Check that only minigame_idle events (if any) are between start and end
      const between = battleEvents.filter((e) => e.measure > minigameStarts[i].measure && e.measure < minigameEnds[i].measure);
      const nonIdle = between.filter((e) => e.type !== "minigame_idle");
      if (nonIdle.length > 0) {
        errors.push(`Only minigame_idle events are allowed between minigame_start at measure ${minigameStarts[i].measure} and minigame_end at measure ${minigameEnds[i].measure}.`);
      }
    }
  }

  // 7. Party jumps matching (can be many, must match Start -> End)
  const partyJumpStarts = events.filter((e) => e.type === "party_jump_start").sort((a, b) => a.beat - b.beat);
  const partyJumpEnds = events.filter((e) => e.type === "party_jump_end").sort((a, b) => a.beat - b.beat);
  if (partyJumpStarts.length !== partyJumpEnds.length) {
    errors.push("Mismatched party_jump_start and party_jump_end events.");
  }
  for (let i = 0; i < Math.min(partyJumpStarts.length, partyJumpEnds.length); i++) {
    if (partyJumpStarts[i].beat >= partyJumpEnds[i].beat) {
      errors.push(`party_jump_start at beat ${partyJumpStarts[i].beat} does not end properly at ${partyJumpEnds[i].beat}.`);
    }
    if (i > 0 && partyJumpStarts[i].beat < partyJumpEnds[i - 1].beat) {
      errors.push(`party_jump_start at beat ${partyJumpStarts[i].beat} overlaps previous party jump.`);
    }
  }
