...
export default function ImportSong() {
  ...
  const handleImport = async () => {
    try {
      ...
      if (!zipPath) return;
      const resolvedZipPath = Array.isArray(zipPath) ? zipPath[0] : zipPath;
      const zipBuffer = await window.electronAPI.readFileBuffer(resolvedZipPath);
      ...
      const resolvedDest = Array.isArray(destFolder) ? destFolder[0] : destFolder;

      // Load zip
      const zip = await JSZip.loadAsync(zipBuffer);

      // Find song name from .ogg or .mid file
      const oggFile = Object.keys(zip.files).find((f) => f.endsWith(".ogg"));
      let songBase = "";
      if (oggFile) {
        songBase = oggFile.replace(/\.ogg$/, "");
      } else {
        throw new Error("Could not determine song name from .ogg or .mid file in zip");
      }
      ...
      await Promise.all(
        Object.keys(zip.files).map(async (filename) => {
          ...
          if (file.dir) return;
          const data = filename.endsWith(".json") || filename.endsWith(".txt") || filename.endsWith(".dta") ? await file.async("string") : await file.async("uint8array");
          if (typeof data === "string") {
            await window.electronAPI.writeFile(join(songFolder, filename), data);
          } else {
            await window.electronAPI.writeFileBuffer(join(songFolder, filename), data);
          }
        }),
      );
  ...
  return (
    <div className="flex flex-col items-center justify-center h-full">
      <OpenSongDialog open={moveDialogOpen} setOpen={handleSongDialog} songPath={songPath} />
