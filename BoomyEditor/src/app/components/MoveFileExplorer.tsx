...
import { ChevronRightIcon, GamepadIcon, DiscIcon, PersonStandingIcon, Search, X } from "lucide-react";
...
interface MoveFileExplorerProps {
  onMoveSelect?: (movePath: string, category: string, song: string, move: string) => void;
}

const GAME_CATEGORIES = ["dc1", "dc2", "dc3", "dc1_dlc", "dc2_dlc", "dc3_dlc", "dcs"];
...
export function MoveFileExplorer({ onMoveSelect }: MoveFileExplorerProps) {
  ...
  useEffect(() => {
    ...
    const initCategories: FileSystemItem[] = GAME_CATEGORIES.map((category) => ({
      name: category,
  ...
  const loadDirectoryContents = async (item: FileSystemItem): Promise<FileSystemItem[]> => {
    try {
  ...
  const findItemInTree = (items: FileSystemItem[], targetPath: string): FileSystemItem | null => {
  ...
  const updateItemInTree = (item: FileSystemItem, targetPath: string, updater: (item: FileSystemItem) => FileSystemItem): FileSystemItem => {
    if (item.path === targetPath) {
      return updater(item);
    }
    ...
    if (item.children) {
      return {
        ...item,
        children: item.children.map((child) => updateItemInTree(child, targetPath, updater)),
      };
    }
  ...
  const handleMoveClick = (item: FileSystemItem) => {
    ...
    const pathParts = item.path.replace(moveLibraryPath + "/", "").split("/");
  ...
  const songFuse = useMemo(() => {
    ...
    const songList = Object.entries(searchIndex.songs_by_name || {}).map(([songKey, songData]) => {
  ...
  const moveFuse = useMemo(() => {
    if (!searchIndex) return null;
    const moveList = Object.entries(searchIndex.moves_by_name || {}).map(([moveKey, moveData]) => {
    ...
    if (searchMode === "song") {
      ...
      songResults.forEach(({ item }) => {
        ...
        if (!grouped[categoryKey]) {
          const matchingCategory = categories.find((c) => c.name === categoryKey);
          if (matchingCategory) {
            grouped[categoryKey] = {
              ...matchingCategory,
              children: [],
        ...
        if (category && !category.children?.find((s) => s.name === songName)) {
          // Find all moves for this song
          const moves: FileSystemItem[] = [];
          if (searchIndex && searchIndex.moves_by_name) {
            Object.entries(searchIndex.moves_by_name).forEach(([moveKey]) => {
          ...
          const song: FileSystemItem = {
            ...
            isExpanded: true,
            isLoaded: true,
            children: moves.sort((a, b) => a.name.localeCompare(b.name)),
          };
  ...
  const renderItem = (item: FileSystemItem, level: number = 0) => {
    ...
    const displayName = isCategory ? CATEGORY_LABELS[item.name as keyof typeof CATEGORY_LABELS] || item.name : item.name;

    return (
      <div key={`${item.path}`} className="max-h-full">
        <div
          className={cn("flex items-center gap-2 py-1 px-2 rounded cursor-pointer hover:bg-muted/50 select-none", level > 0 && "ml-4", isMove && "hover:bg-primary/10")}
        ...
        >
          {item.isDirectory && !isMove && <ChevronRightIcon className={cn("h-4 w-4 transition-transform text-muted-foreground", item.isExpanded && "rotate-90")} />}
          {!item.isDirectory || isMove ? (
            <PersonStandingIcon className="h-4 w-4 text-muted-foreground" />
          ) : isSong ? (
            <DiscIcon className={cn("h-4 w-4", item.isExpanded ? "text-primary" : "text-muted-foreground")} />
          ) : (
            <GamepadIcon className={cn("h-4 w-4", item.isExpanded ? "text-primary" : "text-muted-foreground")} />
          )}
          <span className={cn("text-sm", isCategory && "font-medium", isSong && "font-normal text-muted-foreground", isMove && "font-light hover:text-primary")}>{displayName}</span>
        </div>

        {item.isExpanded && item.children && <div>{item.children.map((child) => renderItem(child, level + 1))}</div>}
      </div>
    );
  };
  ...
  if (!moveLibraryPath) {
    return (
      <div className="p-4 text-center text-muted-foreground">
        ...
        <p className="text-xs mt-1">Load a song with a move library to browse moves</p>
      </div>
    );
  }
  ...
  return (
    <div className="h-full flex flex-col overflow-hidden">
      ...
      <div className="flex-shrink-0 p-4 border-b space-y-3">
        <h3 className="text-sm font-semibold text-muted-foreground">Move Library</h3>

        {/* Search mode selector */}
        <div className="flex gap-2 mb-2">
          <Button type="button" variant={searchMode === "move" ? "default" : "secondary"} onClick={() => setSearchMode("move")}>
            Search by Move
          </Button>
          <Button type="button" variant={searchMode === "song" ? "default" : "secondary"} onClick={() => setSearchMode("song")}>
            Search by Song
          </Button>
        </div>
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder={searchMode === "move" ? "Search moves..." : "Search songs..."}
            value={pendingQuery}
          ...
          />
          <button onClick={() => setSearchQuery(pendingQuery)} className="absolute right-8 top-1/2 transform -translate-y-1/2 p-1 rounded hover:bg-muted transition-colors" title="Search">
        ...
        <div className="text-xs text-muted-foreground">
          {loading && "Loading..."}
          {!loading && (searchQuery ? (filteredMoveCount > 0 ? `${filteredMoveCount} of ${totalMoveCount} moves found` : `No results found for "${searchQuery}"`) : `${totalMoveCount} moves total`)}
        </div>
      </div>
        ...
        {searchQuery && filteredMoveCount === 0 ? (
          <div className="h-full flex items-center justify-center">
            <div className="text-center text-muted-foreground">
              ...
              <div className="text-xs mt-1">Try searching for {searchMode === "move" ? "move names" : "song names"} or switch search mode.</div>
            </div>
          </div>
        ) : (
          <div className="space-y-1">{filteredCategories.map((category) => renderItem(category, 0))}</div>
        )}
      </div>
    </div>
  );
