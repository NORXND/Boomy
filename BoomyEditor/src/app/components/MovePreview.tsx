...
export function MovePreview({
	...
	const handleClipSelect = (clipName: string) => {
		...
		console.log(
			'Added clip to library:',
			`${category}/${song}/${move}/${clipName}`
		);
	};
			...
			<div className="space-y-4">
					...
					{imageUrl ? (
						<div className="flex-shrink-0">
							<img
								...
								className="w-44 h-22 object-cover rounded-lg border"
				...
				<div>
					<h4 className="text-sm font-medium mb-2">
						Clips ({Object.keys(moveData.clips).length})
					</h4>
					<div className="space-y-2">
						{Object.entries(moveData.clips).map(
							([clipName, clip]) => (
								<div
									key={clipName}
									className={cn(
									...
									onClick={() => handleClipSelect(clipName)}
								>
