...
export function NewSongDialog({ open, setOpen }: { open: boolean; setOpen: (open: boolean) => void }) {
  ...
  const handleSongCreation = async () => {
    try {
      ...
      await loadSong(songData.songData, songData.songPath, songData.songName);
      setOpen(false);
      navigate("/editor");
    } catch (error) {
    ...
    >
      <DialogContent className="sm:max-w-md">
        <DialogHeader>
          ...
          <DialogDescription>
            {step === 1 && "Choose where to create your new song folder."}
            {step === 2 && "Enter a name for your new song."}
            {step === 3 && (
              <>
                You can download it from{" "}
                <button className="underline cursor-pointer" onClick={() => window.electronAPI.openExternal("https://github.com/NORXND/milo-move-library")}>
                  milo-move-library
                </button>{" "}
                repository.
              </>
        ...
        {step === 1 && (
          <div className="flex items-center gap-2">
            <div className="flex flex-1 gap-2 flex-row">
              <Input placeholder="Root directory" onChange={(e) => setRootDir(e.target.value)} value={rootDir} />
              <Button variant="secondary" onClick={handleRootDirSelection}>
                Select
              </Button>
            </div>
          </div>
              ...
              <div className="flex flex-1 gap-2 flex-row">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    const cleaned = e.target.value.replace(/[^a-zA-Z0-9]/g, "").toLowerCase();
                    setSongName(cleaned);
                  }}
                />
              </div>
            </div>
            <div className="text-xs text-muted-foreground mt-1">Song name must be lowercase ASCII letters and numbers only.</div>
          </>
        )}
        ...
        {step === 3 && (
          <div className="flex items-center gap-2">
            <div className="flex flex-1 gap-2 flex-row">
              ...
              <Input id="movelib" placeholder="milo-move-library" onChange={(e) => setMoveLibPath(e.target.value)} value={moveLibPath} />
              <Button variant="secondary" onClick={handleMoveLibSelection}>
                Select
              </Button>
            </div>
          </div>
        ...
        <DialogFooter className="sm:justify-between">
          ...
          <div>
            {step === 1 && rootDir && <Button onClick={() => setStep(2)}>Next</Button>}
            {step === 2 && (
              <Button onClick={handleNext} disabled={!songName.trim()}>
                Next
              </Button>
            )}
            {step === 3 && <Button onClick={handleSongCreation}>Create Song</Button>}
          </div>
        </DialogFooter>
      </DialogContent>
    </Dialog>
