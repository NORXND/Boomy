...
export async function openSong(songPath: string, imported: boolean = false) {
  const dotBoomyExists = await window.electronAPI.pathExists(path.join(songPath, ".boomy"));
  ...
  switch (dotBoomyContent) {
    ...
    case "song2":
      toast.error("Songs in 2nd revision are not longer supported.", {
        description: "Converting is not supported yet. Please downgrade your Boomy version if you need to open that song.",
      });
      break;
    case "song1":
...
export async function createSong(rootDir: string, songName: string, moveLibPath: string) {
  ...
  try {
    ...
    const moveVersionExists = await window.electronAPI.pathExists(path.join(moveLibPath, ".boomy"));

    if (moveVersionExists) {
      const dotBoomyContent = await window.electronAPI.readFile(path.join(moveLibPath, ".boomy"));
    ...
    const songVersionExist = await window.electronAPI.pathExists(path.join(songDir, ".boomy"));
    ...
    const templateOgg = new Uint8Array(await templateOggResponse.arrayBuffer());
    const templateCover = new Uint8Array(await templateCoverResponse.arrayBuffer());

    // Write with song name
    await window.electronAPI.writeFileBuffer(path.join(songDir, `${songName}.ogg`), templateOgg);
    await window.electronAPI.writeFileBuffer(path.join(songDir, `${songName}_keep.png`), templateCover);
    ...
    await window.electronAPI.writeFile(path.join(songDir, "song.json"), JSON.stringify(songData, null, 2));

    await window.electronAPI.writeFile(path.join(songDir, ".boomy"), "song3");
